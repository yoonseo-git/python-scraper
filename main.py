...
class WeworkScapper:
    ...
    def __init__(self):
        self.all_jobs = [] # 수집한 job 데이터를 저장하는 리스트 (인스턴스 변수)
    ...
    def scrape_page(self, url):
        ...
        # find() 실패 시 None 반환 -> None 체크로 예외 예방
        for job in jobs:
            ...
            # 각 job 데이터를 딕셔너리로 구조화 후 리스트에 추가
            job_data = {
    ...
    def scrape(self, total_pages):
        # total_pages 만큼 반복하며 각 페이지 스크랩
        for x in range(total_pages):
            url = f"https://weworkremotely.com/remote-contract-jobs?page={x+1}"
            self.scrape_page(url)
        print(len(self.all_jobs)) # 전체 수집된 job 수 출력

    def get_page(self, url):
        response = requests.get(url)
        soup = BeautifulSoup(response.content, "html.parser")
        # pagination에서 page span 개수 = 총 페이지 수
        return len(soup.find("div", class_="pagination").find_all("span", class_="page"))

# WeworkScapper 인스턴스 생성
scapper = WeworkScapper()
# 총 페이지 수 가져오기
total_pages = scapper.get_page("https://weworkremotely.com/remote-contract-jobs?page=1")
# 전체 페이지 스크랩 시작
scapper.scrape(total_pages)
